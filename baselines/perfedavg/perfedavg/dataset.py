...
def _shuffle_data(dataset: Dataset, seed: int) -> Subset:
    g = torch.Generator()
    g.manual_seed(seed)
    shuffled_idx = torch.randperm(len(dataset), generator=g)
...
def load_datasets(
    ...
    trainloaders = [
        DataLoader(dset, batch_size=batch_size, shuffle=True) for dset in trainset_split
    ]
